...
namespace EliteAPI.Classes
{
	public class Commodity
	{
		...
		public Commodity(JObject jo, bool market = false)
		{
			if (market)
				FromJsonMarket(jo);
			else
		...
		public Commodity(Commodity other)             // main fields copied, not the extra data ones
		{
			id = other.id;
		...
		public static void Sort(List<Commodity> list)
		{
			list.Sort(delegate (Commodity left, Commodity right)
			{
		...
		public static List<Commodity> Merge(List<Commodity> left, List<Commodity> right, string otherstation)
		{
			List<Commodity> merged = new List<Commodity>();

			foreach (Commodity l in left)
			{
				Commodity m = new Commodity(l);
				Commodity r = right.Find(x => x.fdname == l.fdname);
				if (r != null)
				{
			...
			foreach (Commodity r in right)
			{
				Commodity m = merged.Find(x => x.fdname == r.fdname);        // see if any in right we have not merged
				if (m == null)
				{
					m = new Commodity(r);
