...
namespace EliteAPI
{
    public class EliteDangerousAPI
    {
		...
		/// Contains the Status.json data, updated upon request.
		/// </summary>
		public JObject MarketData { get { return JObject.Parse(File.ReadAllText(_playerJournalDirectory.GetFiles().Where(x => x.Name == "Market.json").First().FullName)); } }
        ...
        public void Start()
        {
			...
			this.MarketEvent += EliteDangerousAPI_MarketEvent;
			this.FSSAllBodiesFoundEvent += EliteDangerousAPI_FSSAllBodiesFoundEvent;
			this.StartJumpEvent += EliteDangerousAPI_StartJumpEvent;
			this.ScreenshotEvent += EliteDangerousAPI_ScreenshotEvent;
		}
        ...
        private void InitProcessLog()
        {
            ...
            while (isRunning)
            {
                tempLogFile = _playerJournalDirectory
                                    .GetFiles("*.log") //Get all files that end with .log.
                                    .OrderByDescending(x => x.LastWriteTime) //Order them by last written.
                                    .First(); //Get the first one of the ordered list.
				...
				)
				{
					...
					this.threadProcessLogFile = new Thread(() => ProcessLog(logFile, true))
					{
        ...
        private void Process(string json)
        {
            ...
            switch (eventName)
            {
				...
				case "Screenshot":
					ScreenshotEvent?.Invoke(this, JsonConvert.DeserializeObject<ScreenshotInfo>(json));
					break;
                ...
                case "Market":
					MarketInfo market = JsonConvert.DeserializeObject<MarketInfo>(json);
					market.ScanCommodities(this.MarketData);
					MarketEvent?.Invoke(this, market);
                    break;
		...
		public EventHandler<ScreenshotInfo> ScreenshotEvent;
		...
		private void EliteDangerousAPI_ScreenshotEvent(object sender, ScreenshotInfo e)
		{
			if (e.System != "" &&
				(lastSystem.Name == "" ||
		...
		private void EliteDangerousAPI_FSSAllBodiesFoundEvent(object sender, FSSAllBodiesFoundInfo e)
		{
			if (e.SystemName != "" &&
				(lastSystem.Name == "" ||
				lastSystem.Name == null)
		...
		private void EliteDangerousAPI_MarketEvent(object sender, MarketInfo e)
		{
			if (e.StarSystem != "" &&
				(lastSystem.Name == "" ||
				lastSystem.Name == null)
